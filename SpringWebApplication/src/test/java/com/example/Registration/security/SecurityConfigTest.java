...
public class SecurityConfigTest {
    ...
    @Test
    public void testAccessToAccountPageWithoutAuthentication() throws Exception {
        mockMvc.perform(get("/account"))
                .andExpect(status().is3xxRedirection()) // Проверяем, что это перенаправление
                .andExpect(header().string("Location", "http://localhost/login")); // Проверяем точное перенаправление
    }


    @Test
    public void testStaticResourcesAccess() throws Exception {
        mockMvc.perform(get("/css/stylesForAccount.css"))
    ...
    public void testLoginSuccessRedirect() throws Exception {
        mockMvc.perform(get("/home"))
                .andExpect(status().is3xxRedirection()); // Без логина редирект на страницу логина
    }

    @Test
    public void testUploadPageAccessWithoutAuthentication() throws Exception {
        mockMvc.perform(get("/upload"))
                .andExpect(status().isOk()); // Доступ без авторизации
    }
